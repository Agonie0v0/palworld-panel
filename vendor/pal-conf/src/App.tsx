...
function App() {
  ...
  useEffect(() => {
    ...
    const handlePanelMessage = (event: MessageEvent) => {
      ...
      if (message?.type === "PALWORLD_PANEL_LOAD_INI" && typeof message.ini === "string") {
        setFileMode(FileMode.INI);
        deserializeEntriesFromIni(message.ini);
      }
  ...
  return (
    <>
      <main
        className={`flex flex-col items-center min-h-screen bg-gray-100 dark:bg-gray-900 p-4 ${embedded ? "pal-conf-embedded" : ""}`}
        onDragOver={(e) => {
          e.preventDefault();
