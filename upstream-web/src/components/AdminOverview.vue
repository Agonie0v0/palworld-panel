...
const copy = computed(() => zh.value ? {
  ...
  coverageHint: "Base Pals with a clear assignment",
...
const coveragePercent = computed(() => workers.value.length ? Math.round((workingCount.value / workers.value.length) * 100) : 0);
  ...
  <div class="command-deck">
    ...
    <section class="habitat-deck" aria-labelledby="habitat-title">
      <header class="deck-heading">
        ...
        <div class="coverage-orb"><strong>{{ coveragePercent }}%</strong><span>{{ copy.coverage }}</span></div>
      </header>
        ...
        <div class="pal-habitat">
          ...
          <div v-if="activeBaseWorkers.length" class="pal-constellation" :aria-busy="loading">
            ...
            >
              ...
              <n-icon class="pal-node__open"><ArrowRight /></n-icon>
            </button>
          </div>
      ...
      <article class="pal-focus" role="dialog" aria-modal="true" :aria-label="copy.focusTitle">
        ...
        <div class="pal-focus__grid">
          ...
          <section class="pal-focus__capabilities">
            ...
            <div><i v-for="work in selectedWorker.workSuitabilities" :key="work.id">{{ workLabel(work) }} <b>Lv.{{ work.level }}</b></i><em v-if="!selectedWorker.workSuitabilities.length">—</em></div>
          </section>
          <section class="pal-focus__passives">
            <span>{{ copy.passives }}</span>
            <div v-if="selectedWorker.passives.length"><article v-for="skill in selectedWorker.passives.slice(0, 4)" :key="skill.id"><strong>{{ skill.name }}</strong><small>{{ skill.description || skill.id }}</small></article></div>
