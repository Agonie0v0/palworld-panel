  ...
  <div class="command-deck">
    <header class="command-deck__intro">
      ...
      <div class="command-deck__actions">
        <DataFreshness class="command-deck__freshness" :timestamp="lastUpdatedAt" :label="copy.updated" />
...
.base-switcher { display: grid; align-content: start; gap: 10px; }
.base-switcher button { position: relative; display: grid; min-height: 84px; grid-template-columns: 38px minmax(0, 1fr) auto; align-items: center; gap: 13px; padding: 14px 16px; color: var(--app-ink); background: var(--app-surface); border: 1px solid var(--app-border); border-radius: 14px; cursor: pointer; text-align: left; transition: transform 140ms var(--ease-out), border-color 160ms ease-out, background-color 160ms ease-out, box-shadow 160ms ease-out; }
.base-switcher button:hover { border-color: color-mix(in srgb, var(--app-accent) 45%, var(--app-border)); box-shadow: 0 6px 18px var(--app-accent-glow); }
.base-switcher button:active { transform: scale(0.97) !important; }
.base-switcher button.active { color: white; background: linear-gradient(135deg, var(--app-accent), color-mix(in srgb, var(--app-accent) 75%, #000)); border-color: var(--app-accent); box-shadow: 0 8px 24px var(--app-accent-glow); }
.base-switcher__index { color: var(--app-ink-muted); font: 700 11px var(--app-font-data); }
.base-switcher button.active .base-switcher__index { color: rgb(255 255 255 / 75%); }
.base-switcher__copy { display: grid; min-width: 0; gap: 4px; }
.base-switcher__copy strong,.base-switcher__copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.base-switcher__copy strong { font-size: 14px; font-weight: 700; }
.base-switcher__copy small { color: var(--app-ink-muted); font-size: 11px; }
.base-switcher button.active small { color: rgb(255 255 255 / 80%); }
.base-switcher__state { display: grid; min-width: 26px; height: 26px; place-items: center; color: var(--app-success); background: var(--app-success-soft); border-radius: 50%; font: 700 10px var(--app-font-data); }
.base-switcher__state.alert { color: var(--app-warning); background: var(--app-warning-soft); }
.base-switcher__empty { padding: 24px; color: var(--app-ink-muted); background: var(--app-surface); border: 1px dashed var(--app-border); border-radius: 14px; font-size: 13px; }
.pal-habitat { min-width: 0; min-height: 500px; padding: clamp(20px, 2vw, 30px); background: var(--app-surface); border: 1px solid var(--app-border); border-radius: 20px; box-shadow: var(--app-shadow); }
.pal-habitat__header { display: flex; align-items: end; justify-content: space-between; gap: 20px; margin-bottom: 20px; }
.pal-habitat__header > div:first-child span { display: flex; align-items: center; gap: 7px; color: var(--app-accent); font-size: 12px; font-weight: 700; }
.pal-habitat__header h4 { margin-top: 5px; font-size: clamp(20px, 1.5vw, 28px); font-weight: 750; }
.habitat-stats { display: flex; gap: 20px; }
.habitat-stats span { display: grid; color: var(--app-ink-muted); font-size: 10px; text-align: right; }
.habitat-stats strong { color: var(--app-ink); font: 740 20px var(--app-font-data); }
.habitat-stats span.alert strong { color: var(--app-warning); }
.pal-constellation { display: grid; grid-template-columns: repeat(3, minmax(230px, 1fr)); gap: 14px; }
.pal-node { position: relative; display: grid; min-height: 186px; grid-template-columns: 96px minmax(0, 1fr); align-items: center; gap: 16px; overflow: hidden; padding: 18px; color: var(--app-ink); background: var(--app-surface-muted); border: 1px solid var(--app-border); border-radius: 16px; box-shadow: var(--app-shadow); cursor: pointer; text-align: left; transition: transform 140ms var(--ease-out), box-shadow 160ms ease-out, border-color 160ms ease-out; }
.pal-node:hover { z-index: 2; border-color: color-mix(in srgb, var(--app-accent) 55%, var(--app-border)); box-shadow: 0 8px 24px var(--app-accent-glow); }
.pal-node:active { transform: scale(0.97) !important; }
.pal-node.attention { border-color: color-mix(in srgb, var(--app-warning) 50%, var(--app-border)); background: linear-gradient(145deg, color-mix(in srgb, var(--app-warning-soft) 40%, var(--app-surface-muted)), var(--app-surface-muted)); }
.pal-node__visual { position: relative; display: grid; width: 96px; height: 112px; place-items: center; }
.pal-node__halo { position: absolute; inset: 12px 2px 0; background: color-mix(in srgb, var(--app-accent) 15%, transparent); border-radius: 48% 48% 18px 18px; transition: background-color 160ms ease-out; }
.pal-node:hover .pal-node__halo { background: color-mix(in srgb, var(--app-accent) 28%, transparent); }
.pal-node__visual img { position: relative; z-index: 1; width: 92px; height: 92px; object-fit: contain; filter: drop-shadow(0 8px 12px rgba(0, 0, 0, 0.25)); transition: transform 200ms var(--ease-spring); }
.pal-node:hover .pal-node__visual img { transform: scale(1.05); }
.pal-node__visual > i { position: absolute; z-index: 2; right: 0; bottom: 0; display: grid; width: 28px; height: 28px; place-items: center; color: white; background: var(--app-ink); border: 2px solid var(--app-surface); border-radius: 50%; font: 700 10px var(--app-font-data); font-style: normal; }
.pal-node__body { display: grid; min-width: 0; }
.pal-node__state { display: flex; align-items: center; gap: 6px; color: var(--app-success); font-size: 11px; font-weight: 700; }
.pal-node__state > i { width: 7px; height: 7px; background: currentColor; border-radius: 50%; }
.pal-node.attention .pal-node__state { color: var(--app-warning); }
.pal-node__body > strong { overflow: hidden; margin-top: 7px; font-size: 15px; font-weight: 700; text-overflow: ellipsis; white-space: nowrap; }
.pal-node__body > small { overflow: hidden; margin-top: 2px; color: var(--app-ink-muted); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
.pal-node__vitals { display: grid; gap: 8px; margin-top: 12px; }
.pal-node__vitals > span { display: grid; grid-template-columns: 16px minmax(0, 1fr) 34px; align-items: center; gap: 7px; color: var(--app-ink-muted); }
.pal-node__vitals .n-icon { font-size: 14px; }
.pal-node__vitals i { height: 5px; overflow: hidden; background: var(--app-border); border-radius: 4px; }
.pal-node__vitals b { display: block; height: 100%; background: var(--app-accent); border-radius: inherit; }
.pal-node__vitals em { font: 600 9px var(--app-font-data); font-style: normal; text-align: right; }
.pal-habitat__empty { display: grid; min-height: 360px; place-content: center; place-items: center; gap: 12px; color: var(--app-ink-muted); font-size: 14px; }
.pal-habitat__empty .n-icon { font-size: 42px; }
...
.pal-node {
  ...
  border-radius: 18px;
  cursor: pointer;
  transition: transform 140ms var(--ease-out), box-shadow 160ms ease-out, border-color 160ms ease-out;
}
.pal-node:hover {
