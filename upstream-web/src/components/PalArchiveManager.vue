...
const useFallback = (event) => { if (event.target.src !== unknownPal) event.target.src = unknownPal; };
const load = async () => {
...
<template>
  ...
  <n-drawer :show="Boolean(selected)" :width="560" placement="right" @update:show="$event || (selected = null)">
    <n-drawer-content v-if="selected" :title="copy.detail" closable>
      <div class="pal-detail-hero"><img :src="palPortrait(selected.palId)" alt="" @error="useFallback" /><div><h2>{{ selected.name }}</h2><p>{{ selected.speciesName }} · Lv.{{ selected.level }} · {{ '★'.repeat(selected.stars) || (zh ? '零星' : 'No stars') }}</p><n-flex><n-tag v-if="selected.lucky" type="warning">{{ copy.lucky }}</n-tag><n-tag v-if="selected.alpha" type="error">{{ copy.alpha }}</n-tag></n-flex></div></div>
...
.pal-detail-hero { display: flex; align-items: center; gap: 18px; margin-bottom: 20px; }.pal-detail-hero img { width: 104px; height: 104px; object-fit: contain; background: var(--app-surface-muted); border-radius: 8px; }.pal-detail-hero h2 { margin: 0; font-size: 22px; }.pal-detail-hero p { margin: 5px 0 10px; color: var(--app-ink-muted); }.detail-section { padding: 17px 0; border-bottom: 1px solid var(--app-border); }.detail-section h3 { margin: 0 0 10px; font-size: 14px; }.detail-tags { display: flex; flex-wrap: wrap; gap: 6px; }.skill-row,.skill-list > div { padding: 10px 12px; background: var(--app-surface-muted); border-radius: 6px; }.skill-row p,.skill-list p { margin: 4px 0 0; color: var(--app-ink-muted); font-size: 11px; line-height: 1.5; }.skill-list { display: grid; gap: 6px; }
...
@media (max-width: 640px) { .pal-summary { grid-template-columns: 1fr 1fr; }.pal-filter-main { grid-template-columns: 1fr; }.advanced-filter { grid-template-columns: 1fr 1fr; }.pal-grid { grid-template-columns: 1fr; }.pal-card { grid-template-columns: 60px minmax(0,1fr) 38px; }.pal-portrait { width: 60px; height: 60px; } }
