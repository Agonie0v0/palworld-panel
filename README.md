...
| 玩家数据 | 图鉴与捕获、传送点、探索区域、头目、地下城、科技点、配方、油田和存档坐标 |
| 帕鲁仓库 | 全服帕鲁头像、归属与终端位置、工作适应性、被动/主动/伙伴技能、星级、个体值和组合筛选 |
| 全服库存 | 物品图标、分类与总量，并定位到玩家/据点/公会、容器、槽位和坐标 |
| 公会与地图 | 公会成员、会长、基地、在线玩家位置和存档中的世界坐标 |
| 世界数据 | 基地、容器与其他原始解析数据，供排查和核对使用 |
| RCON 与广播 | 命令执行、模板、批量导入、玩家/物品/帕鲁占位符、定时任务和广播模板 |
| 存档与备份 | 本机目录、ZIP 导入、Agent 同步、备份创建/校验/下载/恢复/删除及 WebDAV |
| 模组与创意工坊 | 在同一页面管理 PAK/配置模组扫描、上传、启用状态，以及 Steam 创意工坊搜索和安装 |
...
| 存档解析器 | 历史玩家与完整进度、公会、基地、地图坐标、帕鲁仓库、工作状态、全服物品和精确容器位置 |
