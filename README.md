...
- 常用游戏服参数、任意高级 `PalWorldSettings.ini` 参数写入和 OptionSettings 导入预览
...
1. 在“控制中心 -> 配置生成器”调整参数并生成配置。
2. 复制生成的完整 `PalWorldSettings.ini`、`OptionSettings=(...)`，或参数列表。
3. 打开“控制中心 -> 服务器参数”，点击“导入配置”并粘贴。
4. 核对变更预览后点击“应用到表单”。
5. 回到服务器参数页面点击“保存”，配置才会写入服务器；重启游戏服后生效。

导入采用合并模式，没有出现在导入内容中的现有参数会继续保留。密码、Token 和 Secret 等敏感值只会以掩码显示。
